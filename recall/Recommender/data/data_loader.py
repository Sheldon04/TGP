...
def _read_ml20m1() -> List[Tuple[int, int, float]]:
    data = []
    with open(os.path.join('D:\\PyCharm\\Recommender-System-c5593a592c2e62b42c1178254e96527f66fbf2af(1)\\归档(3)\\ratings-ucf-concat-7.csv'), 'r') as f:
    # with open(os.path.join('/Users/sheldon-t/Desktop/nosql/usercf/ratings-test.csv'), 'r') as f:
        for line in f.readlines()[1:]:
            values = line.strip().split(',')
            user_id, movie_id, rating = int(values[0]), int(values[1]), float(values[2])
...
def ml20m1() -> List[Tuple[int, int, float, int]]:
    return _load_data(_read_ml20m1, 20000263, 138493, 26744, 'ml20m')
