...
def _train_graph(model, train_ds, test_ds, topk_data, optimizer, loss_object, epochs, userMap, itemMap):
    ...
    for epoch in range(epochs):
        ...
        if epoch == epochs:
            print("zhl")
            topk(topk_data, score_fn, userMap=userMap, itemMap=itemMap)



def _train_eager(model, train_ds, test_ds, topk_data, optimizer, loss_object, epochs, userMap, itemMap):
    ...
    model.fit(train_ds, epochs=epochs, verbose=0, validation_data=test_ds,
              callbacks=[RsCallback(topk_data, get_score_fn(model), userMap=userMap, itemMap=itemMap)])


class RsCallback(tf.keras.callbacks.Callback):
    def __init__(self, topk_data: TopkData, score_fn: Callable[[Dict[str, List[int]]], List[float]], userMap=None, itemMap=None):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch == 9:
            # print("caonima")
            topk(self.topk_data, self.score_fn, userMap=self.userMap, itemMap=self.itemMap)
...
def train(model: tf.keras.Model, train_data: List[Tuple[int, int, int]], test_data: List[Tuple[int, int, int]],
          topk_data: TopkData, optimizer=None, loss_object=None, epochs=100, batch=512, execution='eager', userMap=None, itemMap=None) -> None:
    """
    通用训练流程。
    ...
    train_fn(model, train_ds, test_ds, topk_data, optimizer, loss_object, epochs, userMap=userMap, itemMap=itemMap)
